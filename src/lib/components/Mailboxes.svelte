	...
	async function handleUpdateMailbox(mailboxId: number) {
		...
		try {
			...
			mailboxes = mailboxes.map((m) =>
				m.id === mailboxId ? { ...m, name: editingMailboxName } : m
			);
		...
		{#each mailboxes as mailbox (mailbox.id)}
			<li>
				{#if editingMailboxId === mailbox.id}
					<input
						type="text"
						bind:value={editingMailboxName}
						onkeydown={(e) => e.key === 'Enter' && handleUpdateMailbox(mailbox.id)}
					/>
					<button onclick={() => handleUpdateMailbox(mailbox.id)}>Save</button>
					<button onclick={() => (editingMailboxId = null)}>Cancel</button>
				{:else}
					<a
						href={`/inbox?mailboxId=${mailbox.id}`}
						class:active={$page.url.searchParams.get('mailboxId') == String(mailbox.id)}
						>{mailbox.name}</a
					>
